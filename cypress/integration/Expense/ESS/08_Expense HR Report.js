...
describe('Expense HR Report', function() {
	...
	it('Navigate HR Claim Report', function() {
		cy.FinanceLogin()		
		cy.visit(Cypress.env('url')+'Expense/Reports/HRReport?Menu=hrreport')
		cy.wait(2000)
	})
	...
	it('Verify Records according claim Status', function() {
		...
		[ "Pending", "Approved", "Rejected", "Finance Approved", "Finance Rejected", "Cancelled"];	
		 ...
		 for(let j=0; j< claimStatus.length; j++){
			cy.get('#bystatus').select(claimStatus[j])
			cy.get('#bystatus').select(claimStatus[j],{force: true})
			cy.get('#bystatus').select(claimStatus[j])
			cy.wait(1000)
			...
			//cy.wait(3000)
			cy.get('#EStableSorter>tbody>tr:nth-child('+num+')>:nth-child(10)').contains(claimStatus[j].split(" ").join(""))
...
/*	it('Verify Records according Employee', function() {
		const { softAssert, softExpect } = chai;
		cy.visit(Cypress.env('url')+'Expense/Reports/HRReport?Menu=hrreport')
		cy.get('[value="Search"]').click()
		cy.wait(5000)
		let Emp =
		[ "Claim test", "Expense Test"];	
		
		 for(let j=0; j< Emp.length; j++){
	
			cy.get('#byempid').select(Emp[j])
				cy.get('#byempid').select(Emp[j],{force: true})
					cy.get('#byempid').select(Emp[j])
			cy.wait(1000)
			cy.xpath("//button[contains(text(),'Search')]").click({force: true})
			cy.wait(3000)
			...
			var num = parseFloat(i+1)
			//cy.wait(3000)
			cy.get('#EStableSorter>tbody>tr:nth-child('+num+')>:nth-child(4)').contains(Emp[j])
...
*/
	it('Verify Records according From Date To Date', function() {
		cy.visit(Cypress.env('url')+'Expense/Reports/HRReport?Menu=hrreport')
		cy.wait(2000)
		cy.xpath("//button[contains(text(),'Search')]").click({force: true})
		...
		cy.wait(3000)
		//cy.get('[name="EStableSorter_length"]').select('100',{force: true})
			cy.get('body').then(($body) => {
			...
			var num = parseFloat(i+1)
			cy.wait(3000)
			cy.get('#EStableSorter>tbody>tr:nth-child('+num+')>:nth-child(6)').contains(currentDate)
	...
	it('Verify Records according Applied Date', function() {
		const { softAssert, softExpect } = chai;
		cy.visit(Cypress.env('url')+'Expense/Reports/HRReport?Menu=hrreport')
		cy.wait(2000)
			cy.xpath("//button[contains(text(),'Search')]").click({force: true})
		...
		cy.get('#EntryDate').click({force: true}).then(input => {
				input[0].dispatchEvent(new Event('input', { bubbles: true }))
				input.val(currentDate)
		})
	...
	it('Verify Records according Expense Date', function() {
		const { softAssert, softExpect } = chai;
		cy.visit(Cypress.env('url')+'Expense/Reports/HRReport?Menu=hrreport')
		cy.wait(2000)
		cy.xpath("//button[contains(text(),'Search')]").click({force: true})
		...
		cy.get('[name="EStableSorter_length"]').select('100',{force: true})
			...
			var num = parseFloat(i+1)
			cy.wait(3000)
			cy.get('#EStableSorter>tbody>tr:nth-child('+num+')>:nth-child(7)').contains(currentDate)
	...
	it('Verify Records according Search functionality', function() {
		...
		cy.wait(5000)
			...
			cy.get('[type="search"]').clear().type('Local')
		
			cy.get('#EStableSorter>tbody>').contains('Local')
			cy.wait(2000)
			cy.get('[type="search"]').click({force: true})
			cy.get('[type="search"]').clear().type('Office')
