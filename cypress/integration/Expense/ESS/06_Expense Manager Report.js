...
describe('Expense Claim Report', function() {
	...
	it('Navigate Expense Manager Report', function() {
		cy.ManagerLogin()		
...
/*	it('Verify Records according Employee', function() {
		...
		[ "Claim", "Expense"];	
		 ...
		 for(let j=0; j< Emp.length; j++){
			 cy.get('#bystatus').select('All',{force: true})
			cy.get('#byempid').select(Emp[j],{force: true})
			
			//cy.get('#byempid').select(contains(Emp[j]),{force: true})
			//cy.get('#byempid').contains(Emp[j]).click({force: true});
