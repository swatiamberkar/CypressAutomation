...
describe('Expense Manager Approval', function() {
	...
	it('Approve all Claim Entry from Manager 2', function() {
	...
	it('Navigate Finance approval Page', function() {
		cy.FinanceLogin()		
		cy.visit(Cypress.env('url')+'/Expense/Transaction/FinanceApproval?Menu=closeclaimentry')
		cy.wait(2000)
	})
	
	it('Verify Validation Massges - Please Select Month', function() {	
		const { softAssert, softExpect } = chai;
		cy.get('#search').click({force: true})
	
		 cy.get(".noty_body").invoke('text').then((text) => {
		 softExpect(text.trim()).to.eq('Please Select Month');
		 cy.wait(500)
			cy.get(".noty_body").click({force: true})
		  })	  		  
	})
	...
	it('Approve Claim Entry through Details Popup', function() {
		...
		cy.get('#EStableSorter > tbody > tr:nth-child(1)>td:nth-child(14)>button').click({force: true})
		cy.xpath('//tbody/tr[1]/td[14]/input[1]').clear().type('Approve through Details Popup');
		cy.get('#Remark').clear().type('Approve from Finance');
	...
	it('Reject Claim Entry through Details Popup', function() {
		...
		cy.get('#EStableSorter > tbody > tr:nth-child(1)>td:nth-child(14)>button').click({force: true})
		cy.xpath('//tbody/tr[1]/td[14]/input[1]').clear().type('Reject through Details Popup');
		cy.get('#Remark').clear().type('Reject from Finance');
	...
	it('Approve all Claim Entry ', function() {
		...
		cy.get('#EStableSorter > tbody > tr >td:nth-child(11)>input').eq(i).clear().type('Approved');
		})
		cy.get('#btnsave').click({force: true})
				//cy.wait(5000)
				cy.get(".noty_body").invoke('text').then((text) => {
	...
	it('Approve all Claim Entry from Manager 1', function() {
		const { softAssert, softExpect } = chai;
		cy.ManagerLogin()		
		cy.visit(Cypress.env('url')+'Expense/Transaction/ExpenseApprovals?Menu=managerapproval')
		cy.wait(2000)	
	...
	it('Reject all Claim Entry from Finance', function() {
		const { softAssert, softExpect } = chai;
		cy.FinanceLogin()		
		cy.visit(Cypress.env('url')+'/Expense/Transaction/FinanceApproval?Menu=closeclaimentry')
		cy.wait(2000)
		...
		cy.get('#EStableSorter > tbody > tr >td:nth-child(11)>input').eq(i).clear().type('Rejected');
		})
		cy.get('#btnsave').click({force: true})
				//cy.wait(5000)
				cy.get(".noty_body").invoke('text').then((text) => {
