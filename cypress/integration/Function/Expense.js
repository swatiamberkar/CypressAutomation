...
class Expense {
				...
				cy.get('#EStableSorter > tbody > tr> td:nth-child(5)').each(($el, index, $list) => {
					if ($el.text() == transcationID.trim()) {
						...
						cy.get('.table-responsive-vertical > #EStableSorter > tbody > tr > :nth-child(4)').eq(0).invoke('text').then((text) => {
							...
							softExpect(text.trim()).to.eq(Status.trim());
						})
						...
						if (managerLevel == 2 || managerLevel == 3) {
							...
							cy.get('.table-responsive-vertical > #EStableSorter > tbody > tr > :nth-child(4)').eq(2).invoke('contain.text').then((text) => {
								//assert.equal(Status, text.trim(), "Status is not match")
								softExpect(text.trim()).to.eq(Status.trim());
							})
						}
	...
	RejectExternallyAtManager(REJECTREMARK) {
		const { softAssert, softExpect } = chai;
		cy.visit(Cypress.env('essUrl') + 'Expense/Transaction/ExpenseApprovals?Menu=managerapproval')
		cy.wait(2000)
			...
			text.forEach(function (entry) {
				...
				cy.get('tbody > tr > :nth-child(10)').each(($el, index, $list) => {
					if ($el.text() == transcationID.trim()) {
						...
						cy.get('tr > [data-title="Reject"] > #rdoReject').eq(index).check();
						cy.get('tr > [data-title="Remarks"] > #txtRemarks').eq(index).clear({ force: true }).type(REJECTREMARK);
						cy.get('#btnsave').click();
						cy.wait(2000)
	...
	RejectFromDetailAtManager(REJECTREMARK, ClaimAmount, ApprovedAmountFromManager) {
		cy.visit(Cypress.env('essUrl') + 'Expense/Transaction/ExpenseApprovals?Menu=managerapproval')
		cy.wait(2000)
			...
			text.forEach(function (entry) {
				...
				cy.get('tbody > tr > :nth-child(10)').each(($el, index, $list) => {
					if ($el.text() == transcationID.trim()) {
						...
						cy.get('#tblDetails > tbody > tr > [data-title="Remarks"] > #txtRemarks').type(REJECTREMARK, { force: true });
						cy.get('#notes').type(REJECTREMARK, { force: true });
	...
	ApproveFromDetailAtFinance(financeRemark, ClaimAmount, ApprovedAmountFromFinance) {
		const { softAssert, softExpect } = chai;
		cy.visit(Cypress.env('essUrl') + 'Expense/Transaction/FinanceApproval?Menu=closeclaimentry')
		cy.wait(2000)
		cy.readFile('cypress/fixtures/Expense/Expense.json').then((text) => {
			text.forEach(function (entry) {
				...
				cy.get('tbody > tr > :nth-child(12)').each(($el, index, $list) => {
					if ($el.text() == transcationID.trim()) {
						...
						if (ClaimAmount != ApprovedAmountFromFinance) {
							cy.get('#txtFinance').clear({ force: true }).type(ApprovedAmountFromFinance, { force: true })
						}
	...
	RejectExternallyAtFinance(financeRemark) {
		const { softAssert, softExpect } = chai;
		cy.visit(Cypress.env('essUrl') + 'Expense/Transaction/FinanceApproval?Menu=closeclaimentry')
		cy.wait(2000)
		cy.readFile('cypress/fixtures/Expense/Expense.json').then((text) => {
	...
	RejectFromDetailAtFinance(financeRemark, ClaimAmount, ApprovedAmountFromFinance) {
		const { softAssert, softExpect } = chai;
		cy.visit(Cypress.env('essUrl') + 'Expense/Transaction/FinanceApproval?Menu=closeclaimentry')
		cy.wait(2000)
		cy.readFile('cypress/fixtures/Expense/Expense.json').then((text) => {
			text.forEach(function (entry) {
				...
				cy.get('tbody > tr > :nth-child(12)').each(($el, index, $list) => {
					if ($el.text() == transcationID.trim()) {
						...
						cy.get('#rejectentry').click({ force: true })
						cy.get(".noty_body").invoke('text').then((text) => {
							softExpect(text.trim()).to.eq("Expense reject and closed successfully.");
							cy.wait(3000)
							//cy.get(".noty_body").click({force: true})
							cy.wait(2000)
						})
	...
	verifyManagerRemark(managerLevel, managerID, managerName, manager1Remark, managerID2, managerName2, manager1Remark2,
		...
		cy.get('#MngrComments > tr > :nth-child(4)').eq(0).should('contain.text', manager1Remark.trim());
	...
	verifyDetailsPopup(employeeID, ClaimDate, ClaimType, ClaimAmount, FromDate, ToDate, Remark,
		...
		cy.readFile('cypress/fixtures/Expense/Expense.json').then((text) => {
			text.forEach(function (entry) {
				...
				cy.get('#claimdata > .table-responsive-vertical > .table > tbody > :nth-child(6) > :nth-child(4)').should('contain.text', REJECTREMARK);
				cy.get('#apprej > .modal-dialog > .modal-content > .modal-footer > .btn').click()
			})
		})
	}

	verifyExpensedetailsAtMyClaimReport(employeeID, employeeName, currentDate, ClaimDate, ClaimType, ClaimAmount, FromDate, ToDate, filePath, Remark,
		FromLocation, ToLocation, Remark1, VendorName, InvoiceNumberId, Mode, Purpose, ApprovedAmountFromManager, ApprovedAmountFromFinance,
		ApproveStatus, REJECTREMARK, managerID, managerName, manager1Remark, manager1Status, managerID2, managerName2, managerRemark2,
		managerID3, managerName3, managerRemark3, managerLevel, financeRemark, financeStatus) {
		const { softAssert, softExpect, softTrue } = chai;
		cy.visit(Cypress.env('essUrl') + 'Expense/Reports/MyExpenseReports?Menu=myclaimreport')
		cy.wait(5000)
		cy.get('#claimprps').select('All')
		cy.get('#dateRange').click({ force: true }).then(input => {
			input[0].dispatchEvent(new Event('input', { bubbles: true }))
			input.val(ClaimDate + ' to ' + ClaimDate)
		})
		...
		cy.readFile('cypress/fixtures/Expense/Expense.json').then((text) => {
			text.forEach(function (entry) {
				var transcationID = entry.transcationID
				cy.get('#EStableSorter>tbody>tr>:nth-child(10)').each(($el, index, $list) => {
					if ($el.text() == transcationID.trim()) {
						cy.get('#EStableSorter>tbody>tr> :nth-child(10)').eq(index).should('contain.text', transcationID);
						cy.get('#EStableSorter>tbody > tr > :nth-child(1)').eq(index).should('contain.text', currentDate);

						cy.get('tbody > tr > :nth-child(2)').eq(index).should('contain.text', ClaimDate);
						cy.get('tbody > tr > :nth-child(3)').eq(index).should('contain.text', ClaimType);
						cy.get('tbody > tr > :nth-child(4) > .tag').eq(index).should('contain.text', financeStatus);
						cy.get('tbody > tr > :nth-child(5)').eq(index).should('contain.text', ClaimAmount);
						cy.get('tbody > tr > :nth-child(6) > p').eq(index).should('contain.text', ApprovedAmountFromManager);
						cy.get('tbody > tr > :nth-child(7) > p').eq(index).should('contain.text', ApprovedAmountFromFinance);


						// cy.get('#\\37 052').click();
						// cy.get('#apprej > .modal-dialog > .modal-content > .modal-header > .close').click();
						// cy.get(':nth-child(1) > :nth-child(8) > .btn').click();
						// cy.get('#remarkModal > .modal-dialog > .modal-content > .modal-header > .close').click();

						cy.get('tbody > tr > :nth-child(8) > .btn').eq(index).click({ force: true })

						cy.get('tbody > tr > :nth-child(11) > .btn').eq(index).click({ force: true })
					}
				})
			})
		})
