describe('Attendence Module ', function() {
	...
	it('Permisssion Settings', function() {
		cy.visit(Cypress.env('url')+'Settings/Employee/Index?module=attendance&submodule=Permission')
		cy.wait(1000)
		cy.xpath("//span[contains(text(),'Attendance Setting')]").click()
		cy.wait(1000)
		cy.get('#permissionTab').click()
		cy.wait(1000)
		cy.get('#ddShift').select('General')

		cy.get('#txtMaxMtsMonthHour').clear().type('1')
		cy.get('#txtMaxMtsMonthMin').clear().type('0')

		cy.get('#txtMaxMtsDayHour').clear().type('4')
		cy.get('#txtMaxMtsDayMin').clear().type('0')

		cy.get('#txtMinMtsDayHour').clear().type('0')
		cy.get('#txtMinMtsDayMin').clear().type('0')

		cy.get('#txtNoPermMonth').clear().type('10')

		//
		cy.get('#ddAutoPerm').select('Yes')
		cy.get('#ddExcPerm').select('Yes')
		cy.get('#ddPermRoundingType').select('Nill')
		cy.get('#ddPermRounding').select('Nill')

		//click on save button
		cy.get('#btnSavePermissionSetting').click()
		cy.wait(1000)
